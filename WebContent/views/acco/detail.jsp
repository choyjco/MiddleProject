               ...
               <div class="col-12 pl-0 d-flex">
                  <div class="col-8 p-0 me-4">
                     ...
                     <div class="maincontenttitle pb-2">
                        <p>
                        <h3 class="fw-semibold mb-1"><%=accoVO.getMemId()%>님이 호스팅하는 집의 방
                        </h3>
                        </p>
                     </div>
                        ...
                        <div>
                           ...
                           <ul class="c14pwv94 dir dir-ltr mt-3">
                              <li class="c31g148 dir dir-ltr">
                                 ...
                                 <div>
                                    <div class="c1192i2a dir dir-ltr ">침대<%=accoVO.getAccoBed()%>개
                                    </div>
                                 </div>
                              </li>
                              <li class="c31g148 dir dir-ltr">
                                 ...
                                 <div>
                                    <div class="c1192i2a dir dir-ltr">방<%=accoVO.getAccoRoom()%>개
                                    </div>
                                 </div>
                              </li>
                              <li class="c31g148 dir dir-ltr">
                                 ...
                                 <div>
                                    <div class="c1192i2a dir dir-ltr">욕실<%=accoVO.getAccoBath()%>개
                                    </div>
                                 </div>
                              </li>
                           </ul>
...
<script>
    var clientKey = 'test_ck_YZ1aOwX7K8m2PAyqBLm3yQxzvNPG'; // 테스트용 클라이언트 키
    // 2. 결제창 SDK 초기화
    ...
    document.querySelector('button.btn-danger').addEventListener('click', function(e) {
      ...
      var resStart = document.getElementById('checkin').value; // 체크인 날짜 가져오기
      var resEnd = document.getElementById('checkout').value; // 체크아웃 날짜 가져오기
      var resPeople = document.getElementById('guest').value; // 예약 인원 가져오기
      
      var paymentOptions = {
        amount: '1', // 결제할 금액
        orderName: '숙박 예약', // 주문명
        orderId: '1234567890', // 주문번호
        ...
        onFail: function(result) {
          ...
          // 여기에 결제 실패 후 수행할 동작을 추가할 수 있습니다.
        },
        
        onClose: function() {
          // 결제 창이 닫혔을 때 처리할 코드
          console.log('결제 창이 닫혔습니다.');
          // 여기에 결제 창 닫힘 후 수행할 동작을 추가할 수 있습니다.
        }
      };
      ...
      xhr.open('POST', '<%=request.getContextPath()%>/res/resinsert.do', true);
      xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
      
      // 전송할 데이터 생성
      var data = 'resStart=' + encodeURIComponent(resStart) + '&resEnd=' + encodeURIComponent(resEnd) + '&resPeople=' + encodeURIComponent(resPeople);
      ...
      xhr.onload = function() {
        if (xhr.status === 200) {
          ...
          // 리다이렉트를 수행합니다.
          // window.location.href = '/res/resinsert.do';
        } else {
          // 요청이 실패한 경우 처리할 코드
          console.log('요청이 실패하였습니다.');
          // 여기에 실패 시 동작을 수행할 수 있습니다.
        }
      };
    });
</script>
   ...
   <div>
      <div class="he87mfr dir dir-ltr">
         <div class="tq51prx dir dir-ltr">
            <section class="tokr9ou dir dir-ltr">
               <h3 tabindex="-1" class="fw-bolder mb-2 mt-5">리뷰</h3>
            </section>
         </div>
      </div>
...
<!-- 				<br> -->
               ...
               <div class="fw-bold row-2"><%=revVO.getMemId()%></div>
               <%
                  SimpleDateFormat sdf = new SimpleDateFormat("yy/MM/dd");
               String reviewDate = sdf.format(revVO.getRevDate());
               %>
               <div class="row-2 ml-2"><%=reviewDate%></div>
				</div>
               <%
                  String star = "";
