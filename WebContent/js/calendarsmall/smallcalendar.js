		...
		function c(passed_month, passed_year, calNum) {
			...
			clickedElement.on("click", function(){
				...
				if (!firstClick) {
					...
					// 추가 : 수정된 부분
					$('#checkin').val(firstClicked.year + '-' + (firstClicked.month + 1) + '-' + firstClicked.date);
					...
					// 추가 : 수정된 부분
					$('#checkout').val(secondClicked.year + '-' + (secondClicked.month + 1) + '-' + secondClicked.date);
				}
				
				// 체크인 및 체크아웃 날짜의 차이 계산
				var checkinDate = new Date(firstClicked.year, firstClicked.month, firstClicked.date);
				var checkoutDate = new Date(secondClicked.year, secondClicked.month, secondClicked.date);
				var timeDiff = checkoutDate.getTime() - checkinDate.getTime();
				var diffDays = Math.ceil(timeDiff / (1000 * 3600 * 24)); // 차이를 일 단위로 변환
